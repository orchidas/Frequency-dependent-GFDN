...
figure('Units','inches', 'Position',[0 0 6.5 2.5],'PaperPositionMode','auto');
...
for k = 1:length(aperture)
     ...
     if k < length(aperture)
        [plotAxes, plotHandles] = plotImpulseResponseMatrix((w/pi)*(fs/2), 20*log10(abs(couplingMatrixFD(:,:,1:nbins))), 'xlogaxis',1, 'xlim',[20,20000],...
        'xlabel', 'Frequency (Hz)', 'ylabel', 'Magnitude (dB)', 'stemFlag', 0, 'colors', colors(k,:) ,'save', 1, 'ylim', [-50,5], 'plotYLabels', 0); grid on;
     else
          [plotAxes, plotHandles] = plotImpulseResponseMatrix((w/pi)*(fs/2), 20*log10(abs(couplingMatrixFD(:,:,1:nbins))), 'xlogaxis',1, 'xlim',[20,20000],...
     'xlabel', 'Frequency (Hz)', 'ylabel', 'Magnitude (dB)', 'stemFlag', 0, 'colors', colors(k,:) ,'save', 1, 'ylim', [-50,5], 'plotYLabels', 1); grid on;
     end
