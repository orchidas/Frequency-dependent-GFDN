function [] = ftgram2N2S( x, fs, figtitle, saveflag )
...
fig = figure('Units','inches', 'Position',[0 0 3.29 1.9],'PaperPositionMode','auto');
...
% nax1p(2) = 0.4; % move upwards to make room for x label
nax1p(4) = 0.35; % make subplots slightly larger
...
nax2p(2) = 0.2; % move upwards to make room for x label
nax2p(4) = 0.35; % make subplots slightly larger
...
yl2p = get(yl2, 'position');
set(yl2,'position', [yl2p(1) 45000 yl2p(3)]);
cbl = get(cb,'Position');
set(cb, 'Position', [cbl(1) cbl(2) cbl(3) cbl(4)*2.55]);
